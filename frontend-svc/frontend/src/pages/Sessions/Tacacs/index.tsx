...
const TacacsBulks: FC<{ server: ResolvedServerData }> = ({ server }) => {
...
const Page: FC = () => {
  ...
  return (
    <PageLayout title="TACACS+ Sessions" fullHeight={false}>
        ...
        >
          {(server: ResolvedData) =>
            Array.isArray(server) ? (
              <ServersGrid servers={server} proto="tacacs" />
            ) : (
              <TacacsBulks server={server} />
            )
          }
        </Await>
      </Suspense>
...
export const tacacsSessionsLoader: LoaderFunction = async ({
  params,
  request,
}) => {
  ...
  return {
    server: queryClient
      ...
      .then((data) => {
        if (params.server) {
          return data.tacacs?.find((s) => s.server === params.server) ?? null;
        }
        return data.tacacs || null;
      }),
  };
};

export { Page as TacacsSessionsPage };
