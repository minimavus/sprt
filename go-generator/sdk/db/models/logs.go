...
type Log struct {
	...
	Loglevel  null.String `boil:"loglevel" json:"loglevel,omitempty" toml:"loglevel" yaml:"loglevel,omitempty"`
	...
	Chunk     null.String `boil:"chunk" json:"chunk,omitempty" toml:"chunk" yaml:"chunk,omitempty"`
...
var LogWhere = struct {
	...
	Loglevel  whereHelpernull_String
	Owner     whereHelpernull_String
	Message   whereHelpernull_String
	Chunk     whereHelpernull_String
}{
	ID:        whereHelperstring{field: "\"logs\".\"id\""},
	Timestamp: whereHelpernull_Time{field: "\"logs\".\"timestamp\""},
	Loglevel:  whereHelpernull_String{field: "\"logs\".\"loglevel\""},
	Owner:     whereHelpernull_String{field: "\"logs\".\"owner\""},
	Message:   whereHelpernull_String{field: "\"logs\".\"message\""},
	Chunk:     whereHelpernull_String{field: "\"logs\".\"chunk\""},
}
...
var (
	logAllColumns            = []string{"id", "timestamp", "loglevel", "owner", "message", "chunk"}
	logColumnsWithoutDefault = []string{"id"}
	logColumnsWithDefault    = []string{"timestamp", "loglevel", "owner", "message", "chunk"}
...
func (o *Log) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(logPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert logs, could not build conflict column list")
			}

			conflict = make([]string, len(logPrimaryKeyColumns))
			copy(conflict, logPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"logs\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (s LogSlice) ToIDMap() map[string]*Log {
	result := make(map[string]*Log, len(s))
	for _, o := range s {
		result[o.ID] = o
	}
	return result
...
func (s LogSlice) FindItemByID(id string) *Log {
	for _, o := range s {
		if o.ID == id {
			return o
		}
...
func (s LogSlice) FindMissingItemIDs(expectedIDs []string) []string {
	if len(s) == 0 {
		return expectedIDs
	}
	result := []string{}
...
func (o LogSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	for _, row := range o {
		...
		if len(wlCols) == len(logAllColumns) {
			break
		}
	}
...
func (o LogSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) (int64, error) {
	...
	for _, row := range o {
		...
		if len(insertCols) == len(logAllColumns) {
			break
		}
	}
